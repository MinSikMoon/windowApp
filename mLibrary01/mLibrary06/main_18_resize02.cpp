...
public:
	...
	int isClosed(POINT mousePoint) {
		...
		if (distanceChecker(mousePoint, p1) < 5) {
			return 1;
		}
		else if (distanceChecker(mousePoint, p2) < 5) {
			return 2;
		}
		else if (distanceChecker(mousePoint, p3) < 5) {
			return 3;
		}
		else if (distanceChecker(mousePoint, p4) < 5) {
			return 4;
		}
		else {
			return -1; //근접한게 없다. 
	...
	void resize(POINT newUpLeft, POINT newDownRight) {
		upLeft.x = newUpLeft.x;
		upLeft.y = newUpLeft.y;
		downRight.x = newDownRight.x;
		downRight.y = newDownRight.y;
		editorWidth = newDownRight.x - newUpLeft.x ;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_LBUTTONDOWN: {
		...
		if (focusedIdx != -1) { //어떤 도형이 선택되어 있고 
			//테스트 리사이즈
			if (orderFlag = Flag::RESIZE_READY) {
				printf("리사이즈 하겠다고 잡음 \n");
				orderFlag = Flag::RESIZE; //리사이즈 선언함.
	...
	case WM_MOUSEMOVE: {
		...
		if (mouse.getGrapped()) { //그리는 중이라면
			...
			InvalidateRect(hwnd, NULL, TRUE);
			   ...
			   //orderFlag = Flag::NOTHING; //혹시 리사이즈 레디면 풀어주기 위해서

			if (focusedIdx != -1) { //어떤 도형이 선택되어 있고 
				POINT tempPoint = { tempX, tempY };
				int tempResizeState = msc.isClosedAt(focusedIdx, tempPoint);
				//현재 포커스된 도형이 있는데, 지금 그 도형의 꼭짓점 근처에 마우스가 도달 했는지 체크해야 한다. 
				printf("is closed = %d \n", tempResizeState);
				if (tempResizeState != -1) { //어디 근처에 도달 했으면 
					orderFlag = Flag::RESIZE_READY; //리사이즈 레디 상태를 선언한다. 아니다. 클릭을 하면 리사이즈 상태로 변한다. 
					g_resizePoint = tempResizeState;
				}
