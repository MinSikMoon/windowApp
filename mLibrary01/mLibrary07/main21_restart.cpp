...
LRESULT CALLBACK WndProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_LBUTTONDOWN: {
		...
		if (orderFlag == Flag::NOTHING) { //클릭했는데, 아무것도 안하는 거면 도형을 선택하는 거다. 
			focusedIdx = msc.whoIsIn(tempPoint);  //1. 컨테이너를 돌면서 선택된 얘를 찾아준다. //아무것도 없으면 -1이다. 
		...
		if (focusedIdx != -1) { //어떤 도형이 선택되어 있으면 움직이거나 사이즈 변경하거나 둘 중 하나다. 
			if (g_resizePoint != -1) { //===== 도형이 선택도 되어있고, 리사이즈Point가 -1이아니다. => 꼭짓점 근처에 마우스가 있다. 
				...
				orderFlag = Flag::RESIZE; //resize로 바꿔준다. 
			}
			else { //===== 도형이 선택은 되어있는데, 리사이즈 포인트는 -1, 즉 꼭짓점 근처가 아니다. => 이동하기 
				if (focusedIdx == msc.whoIsIn(tempPoint)) { // 그렇더라도 마우스가 현재 선택된  도형 위에 있어야지 이동이 된다. 
					g_oldX = tempX;
					g_oldY = tempY;
	...
	case WM_MOUSEMOVE: {
		...
		if (mouse.getGrapped()) { 
			printf("클릭상태로 마우스 움직임 orderflag = %d \n", orderFlag);
